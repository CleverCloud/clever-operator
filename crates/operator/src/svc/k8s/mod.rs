...
/// provides two method which is given to a kubernetes controller
/// [`Controller<T>`]
        ...
        + 'static,
{
    ...
    fn reconcile(
        obj: Arc<T>,
        ctx: Arc<Context>,
    ) -> impl Future<Output = Result<Action, Self::Error>> + Send + Unpin {
        Box::pin(async move {
            ...
            loop {
                ...
                match stream.try_next().await {
                    ...
                    Ok(Some((obj, _action))) => {
                        info!(
                            kind = &api_resource.kind,
                            namespace = obj.namespace.as_deref().unwrap_or("<none>"),
                            name = obj.name,
